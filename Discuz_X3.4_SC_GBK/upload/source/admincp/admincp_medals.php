...
if (!$operation) {
    ...
    if (!submitcheck('medalsubmit')) {
        ...
        showsubtitle(array('', 'medals_category', 'display_order', 'available', 'name', 'description', 'medals_image', 'medals_type', ''));

        ?>
        <script type="text/JavaScript">
            <?php
            $newCategorySelectStr = '<select name="newcategory[]">'; //"
            foreach ($categoryArr as $arr) {
                $newCategorySelectStr = $newCategorySelectStr . '<option value=' . $arr['id'] . '>' . $arr['name'] . '</option>';
            }
            $newCategorySelectStr = $newCategorySelectStr . "</select>";
            ?>
            //let a = ;
            //alert(a);
            var rowtypedata = [
                [
                    [1, '', 'td25'],
                    [1, '<?php echo $newCategorySelectStr;?>'],
        ...
        foreach (C::t('forum_medal')->fetch_all_data() as $medal) {
            ...
            $categorySelectStr = "<select name='category[$medal[medalid]]'>";

            foreach ($categoryArr as $arr) {
                if ($arr['id'] == $medal[category]) {
                    $categorySelectStr = $categorySelectStr . '<option value=' . $arr['id'] . ' selected>' . $arr['name'] . '</option>';
                } else {
                    $categorySelectStr = $categorySelectStr . '<option value=' . $arr['id'] . '>' . $arr["name"] . '</option>';
                }
            }
            $categorySelectStr = $categorySelectStr . "</select>";
//                <!-- 比较id是否一致,一致放selected默认选中 -->
        ...
        if (is_array($_GET['name'])) {
            foreach ($_GET['name'] as $id => $val) {
                ...
                if ($_GET['category'][$id]) {
                    $update['category'] = dhtmlspecialchars($_GET['category'][$id]);
                }
            ...
            foreach ($_GET['newname'] as $key => $value) {
                if ($value != '' && $_GET['newimage'][$key] != '') {
                    $data = array('name' => dhtmlspecialchars($value),
                        ...
                        'category' => dhtmlspecialchars($_GET['newcategory'][$key]),
                    );
    ...
    if (!submitcheck('medaleditsubmit')) {
        ...
        <tr>
            <td colspan="2">
                ...
                <textarea name="formulapermnew" id="formulapermnew" style="width: 80%" rows="3" onkeyup="formulaexp()"
